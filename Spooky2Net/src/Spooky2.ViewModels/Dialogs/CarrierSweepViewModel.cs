...
public partial class CarrierSweepViewModel : ObservableObject
{
    private readonly ICarrierSweepService _carrierSweepService;

    public CarrierSweepViewModel(ICarrierSweepService carrierSweepService)
    {
    ...
    private async Task CreateSweep()
    {
        ...
        var program = await _carrierSweepService.CreateCarrierSweep(parameters);
        ResultDisplay = $"Created program '{program.Name}' with {program.Frequencies.Count} frequencies";
    }
}
