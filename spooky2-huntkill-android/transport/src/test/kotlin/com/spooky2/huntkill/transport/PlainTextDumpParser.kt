 ...
 * used to drive [com.spooky2.huntkill.transport.fake.FakeTransport].
 *
 * Ported from the `core` test helper `PlainTextDumpParser` so the transport module's
 * tests can build a [FakeTransport] from the same recorded `FullHuntAndKill` session
 * without depending on `core`'s test source set.
 */
